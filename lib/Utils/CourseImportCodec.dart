...
class CourseImportCodec {
  static Map<String, dynamic> dbCourseToOnlineCourseMap(Map<String, dynamic> row) {
  ...
  static Map<String, dynamic> onlineCourseToDbMap(
    ...
    return <String, dynamic>{
      ...
      columnClassNumber: (courseMap['class_number'] ?? courseMap['classNumber'] ?? '').toString(),
      columnTeacher: (courseMap['teacher'] ?? '').toString(),
      columnTestTime: (courseMap['test_time'] ?? courseMap['testTime'] ?? '').toString(),
      columnTestLocation: (courseMap['test_location'] ?? courseMap['testLocation'] ?? '').toString(),
      ...
      columnWeekTime: _toInt(courseMap['week_time'] ?? courseMap['weekTime'], 0),
      columnStartTime: _toInt(courseMap['start_time'] ?? courseMap['startTime'], 0),
      columnTimeCount: _toInt(courseMap['time_count'] ?? courseMap['timeCount'], 0),
      columnImportType: _toInt(courseMap['import_type'] ?? courseMap['importType'], 1),
      // Align with online import behavior: do not carry explicit color.
      columnColor: null,
      columnCourseId: _toNullableInt(courseMap['course_id'] ?? courseMap['courseId']),
    };
  }
