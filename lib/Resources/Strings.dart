class Strings {
  static final String app_name = "南哪课表";
  static final String subtitle_pre = "第";
  static final String subtitle_suf = "周";

  static final String import_manually_title = "手动添加";
  static final String import_manually_subtitle = "手动添加课程表数据";
  static final String import_from_NJU_title = '导入南京大学课表';
  static final String import_from_NJU_subtitle = '登录南京大学教务系统导入课程表';
  static final String manage_table_title = "课表管理";
  static final String manage_table_subtitle = "添加或删除课表数据";
  static final String change_theme_title = "修改主题";
  static final String shuffle_color_pool_title = "重置课程颜色";
  static final String shuffle_color_pool_subtitle = "重置课程颜色池";
  static final String change_week_title = "修改当前周";
  static final String change_week_subtitle = "当前周数：";
  static final String report_title = "反馈";
  static final String report_subtitle = "加入用户群一起愉快地玩耍吧！\n轻触直接加群，长按复制群号";
  static final String donate_title = "投喂";
  static final String donate_subtitle = "给傻翠买支棒棒糖吧！";
  static final String about_title = "关于";
  static final String about_subtitle = "";

  static final String add_table_title = "请输入课程表名称";
  static final String ok = "确认";
  static final String cancel = "取消";

  static final String default_class_table = "默认课表";
}